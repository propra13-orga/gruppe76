...
public class gameplay{
	...
	/** Inventar Sub-Menu ist offen*/
	private boolean invSub = false;
	
	/** Shop offen */
	private boolean shop = false;	
	/** Shop Sub-Menu ist offen */
	private boolean shopSub = false; // 
	
	/** Rechte Mousetaste wurde gedrückt! */
	private boolean mouseR = false;  //
	
	/** Linke Mousetaste wurde gedrückt! */
	private boolean mouseL = false;  //
	
	/** Coords für Inventar Sub-Menu */
	private vec invSubXY = null;  //
	
	/** Player geht nach Rechts */
	private boolean moveD = false;  //
	
	/** Zähler für Pic-Array */
	private int moveDCount = 0;  //
	
	/** Player geht nach Oben */
	private boolean moveW = false;  //
	
	/** Zähler für Pic-Array */
	private int moveWCount = 0;  //
	
	/** Player geht nach Links */
	private boolean moveA = false;  //
	
	/** Zähler für Pic-Array */
	private int moveACount = 0;  //
	
	/** Player geht nach Unten */
	private boolean moveS = false;  //
	
	/** Zähler für Pic-Array */
	private int moveSCount = 0;  //
	
	/** inialisieren der Werte */
	public void init(String file){
		...
		tileset[22] = J2D.createSprite("tiles/NPC.png");
		...
		playerPics[40] = J2D.createSprite("tiles/enemy_fire.png");
		playerPics[41] = J2D.createSprite("tiles/enemy_ice.png");
		playerPics[42] = J2D.createSprite("tiles/enemy_boss.png");
		
		
	}
	
	
	/** Den aktuellen Frame rendern */
	public long render(){
		this.curlvl.getPlayer().addMP();
		
		/**Startzeit des Frames	 */
		...
		if(epsilon == 7){ npc = true;}
		//if(npc){
		//	J2D.drawTextCenter(640, 480, "Möge die Macht mit dir sein Luke!");
		//}
		...
		for(int i=0; i<curlvl.getE().size(); i++){
			...
			if(curlvl.getPlayer().getPos().getZ()==e.getRoom()){
				...
				case STANDARD:
					...
					break;
				case EIS:
					J2D.drawSprite(playerPics[41], e.getPos().getX()*64, e.getPos().getY()*48);
					break;
				case ENDGEGNER:
					J2D.drawSprite(playerPics[42], e.getPos().getX()*64, e.getPos().getY()*48);
					break;
				case FEUER:
					J2D.drawSprite(playerPics[40], e.getPos().getX()*64, e.getPos().getY()*48);
					break;
				default:
					J2D.drawSprite(playerPics[24], e.getPos().getX()*64, e.getPos().getY()*48);
					break;
		...
		// NPC malen
		J2D.drawSprite(tileset[22], curlvl.getShop().getPos().getX(), curlvl.getShop().getPos().getY() );
	...
	/**
	 * Malt die Statusleiste unten
	 * @param npc ob ein NPC in der Nähe ist
	 */
	private void drawGuiBar(boolean npc){
	    // GUI rendern
		...
		//NPC Nachricht anzeigen
		if(npc){
			J2D.setFont(f);
			J2D.drawText(480,754,"Möge die Macht mit dir sein, " + curlvl.getPlayer().name);
		}
	...
	/**
	 * Liegt ein Punkt (Ort des Klickens) im Bereich
	 * @param xRangeMin x wert min
	 * @param xRangeMax x wert max
	 * @param yRangeMin y wert min
	 * @param yRangeMax y wert max
	 * @param x	x-Koordinate zu prüfen
	 * @param y y-Koordinate zu prüfen
	 * @return
	 */
	...
	/**Tile[] Nummer ermitteln für Kreuzungen etc. von Wänden*/
